...
if __name__ == "__main__":
    ...
    parser.add_argument("-i", "--input", default="in.xml", help="Input orignal openvino model.")
    parser.add_argument("-o", "--output", default="out.py", help="Converted python script file name")
    ...
    f.write(f"")
    f.write(f"# auto generated by ir2py from {input_model_fn}")

    f.write(headers)
    func_name = translate(model, "", f)
    ...
    f.write(tail.format(input_model_fn, func_name, os.path.join(new_base, filename)))
    f.close()
